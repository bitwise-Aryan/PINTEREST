...
//     ...pin.toObject(),
//     imageUrl: `${process.env.IK_URL_ENDPOINT}/${pin.media}` // full public URL
// }));
...
//     const pinWithUrl = {
//     ...pin.toObject(),
//     imageUrl: `${process.env.IK_URL_ENDPOINT}/${pin.media}`
// };
...
const pineconeIndex = pc.index('pinterest-pins');
...
export const getPins = async (req, res) => {
    try {
        ...
        else if (boardId) query = { board: boardId };
        else if (savedUserId) {
            const savedPins = await Save.find({ user: savedUserId }).select("pin");
        ...
        // ✅ Fix: Construct full ImageKit URLs
        const pinsWithUrls = pins.map(pin => ({
            ...pin.toObject(),
            imageUrl: pin.media ? `${process.env.IK_URL_ENDPOINT}/${pin.media}` : null
        }));

        res.status(200).json({ pins: pinsWithUrls, nextCursor: hasNextPage ? pageNumber + 1 : null });
    } catch (err) {
        console.error(err);
...
export const getPin = async (req, res) => {
    try {
        ...
        // ✅ Fix: Construct full ImageKit URL
        pin.imageUrl = pin.media ? `${process.env.IK_URL_ENDPOINT}/${pin.media}` : null;

        res.status(200).json(pin);
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: "Failed to fetch pin.", error: err.message });
    }
};
...
export const createPin = async (req, res) => {
    try {
        ...
        if (!authenticatedUserId) return res.status(401).json({ message: "Authentication required." });
        if (!title || !description || !media) return res.status(400).json({ message: "All fields are required!" });
        ...
        const height = metadata.width / clientAspectRatio;
        ...
        let croppingStrategy = "";
        if (parsedCanvasOptions.size !== "original") {
            if (originalAspectRatio > clientAspectRatio) croppingStrategy = ",cm-pad_resize";
        } else {
            if (originalOrientation === "landscape" && parsedCanvasOptions.orientation === "portrait") croppingStrategy = ",cm-pad_resize";
        }

        const transformationString = `w-${width},h-${height}${croppingStrategy},bg-${parsedCanvasOptions.backgroundColor.substring(1)}${
            parsedTextOptions.text ? `,l-text,i-${parsedTextOptions.text},fs-${parsedTextOptions.fontSize*2.1},lx-${textLeftPosition},ly-${textTopPosition},co-${parsedTextOptions.color.substring(1)},l-end` : ""
        }`;

        // ✅ FIX: Use uploadResponse correctly
        ...
        if (!uploadResponse.filePath) throw new Error("ImageKit upload failed.");

        let newBoardId;
        if (newBoard) {
            const boardDoc = await Board.create({ title: newBoard, user: authenticatedUserId });
            newBoardId = boardDoc._id;
        }

        // AI tagging & vector embedding
        let aiTags = [];
        let imageVector = [];
        try {
            if (process.env.CLARIFAI_API_KEY) {
                ...
                const clarifaiResponse = await new Promise((resolve, reject) => {
                    stub.PostWorkflowResults(
                        { workflow_id: "tag-and-embed", inputs: [{ data: { image: { url: uploadResponse.url } } }] },
                        metadata,
                ...
                if (clarifaiResponse.status.code === 10000) {
                    const concepts = clarifaiResponse.results?.[0]?.outputs?.[0]?.data?.concepts ?? [];
                    aiTags = concepts.filter(c => c.value > 0.5).map(c => c.name);

                    const embedding = clarifaiResponse.results?.[0]?.outputs?.[1]?.data?.embeddings?.[0];
                    if (embedding) imageVector = embedding.vector;

                    if (imageVector.length) await pineconeIndex.upsert([{ id: uploadResponse.fileId, values: imageVector }]);
                }
            }
        } catch (err) {
            console.error("AI tagging/vector failed:", err);
        }
        ...
        console.error(err);
        res.status(500).json({ message: "Pin creation failed.", error: err.message });
    }
};
...
export const interactionCheck = async (req, res) => {
    try {
        ...
        if (!token) return res.status(200).json({ likeCount, isLiked: false, isSaved: false });

        jwt.verify(token, process.env.JWT_SECRET, async (err, payload) => {
            if (err) return res.status(200).json({ likeCount, isLiked: false, isSaved: false });

            const userId = payload.userId || payload.id;
            const isLiked = await Like.findOne({ user: userId, pin: id });
            const isSaved = await Save.findOne({ user: userId, pin: id });

            res.status(200).json({ likeCount, isLiked: !!isLiked, isSaved: !!isSaved });
        });
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: "Failed to check interaction.", error: err.message });
    }
};
...
//             if (liked) await Like.deleteOne({ pin: id, user: userId });
//             else await Like.create({ pin: id, user: userId });
//         } else if (type === "save") {
//             const saved = await Save.findOne({ pin: id, user: userId });
//             if (saved) await Save.deleteOne({ pin: id, user: userId });
//             else await Save.create({ pin: id, user: userId });
//         } else if (type === "share") {
//             await Pin.findByIdAndUpdate(id, { $inc: { shares: 1 } });
//         }
...
//         if (type === "like") {
//             const liked = await Like.findOne({ pin: id, user: userId });
//             const pin = await Pin.findById(id).select("user"); // Get Pin owner
...
//                 // Create a new like notification (only if not liking your own pin)
...
//                         type: "like",
//                         pin: id,
//                         content: "liked your pin.", // Generic content for like notification
//                     });
//                 }
//             }
//         } else if (type === "save") {
...
//             else await Save.create({ pin: id, user: userId });
//         } else if (type === "share") {
//             // Assuming 'shares' is a field on the Pin model for basic tracking
//             await Pin.findByIdAndUpdate(id, { $inc: { shares: 1 } }); 
//         }
...
export const interact = async (req, res) => {
    try {
        ...
        if (type === "like") {
            ...
            // Fetch the pin to get the owner ID, needed for the notification logic
            const pin = await Pin.findById(id).select("user"); 
            ...
            if (liked) {
                ...
                if (pinOwnerId !== userId.toString()) {
                    await Notification.create({
                        ...
                        type: "like",
                        pin: id,
                        // FIX: Ensure content is defined to prevent Mongoose validation errors.
        ...
        // IMPORTANT: Add detailed logging here to catch Mongoose validation failures
...
export const getPopularTags = async (req, res) => {
    try {
        ...
        res.status(200).json(popularTags.map(t => t.tag));
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: "Failed to fetch popular tags.", error: err.message });
    }
};
...
export const getTrendingPins = async (req, res) => {
    try {
        ...
        const trendingPins = await Pin.aggregate([
            ...
            { $lookup: { from: "likes", localField: "_id", foreignField: "pin", as: "likeDocs" } },
            { $lookup: { from: "comments", localField: "_id", foreignField: "pin", as: "commentDocs" } },
            { $addFields: { trendingScore: { $add: [ { $size: "$likeDocs" }, { $multiply: [ { $size: "$commentDocs" }, 2 ] } ] } } },
            { $sort: { trendingScore: -1 } },
            { $limit: 20 }
        ]);
        ...
        console.error(err);
        res.status(500).json({ message: "Failed to fetch trending pins.", error: err.message });
    }
};
...
export const getRelatedTags = async (req, res) => {
    try {
        ...
        const likedPins = await Like.find({ user: userId }).select("pin");
        const savedPins = await Save.find({ user: userId }).select("pin");
        const pinIds = [...likedPins.map(p => p.pin), ...savedPins.map(p => p.pin)];

        if (!pinIds.length) return res.status(200).json([]);
        ...
        res.status(200).json(relatedTags.map(t => t.tag).filter(Boolean));
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: "Failed to fetch related tags.", error: err.message });
    }
};
...
export const getSimilarPins = async (req, res) => {
    try {
        ...
        const fetchResponse = await pineconeIndex.fetch([id]);
        const sourceVector = fetchResponse.records[id]?.values;
        if (!sourceVector) return res.status(404).json({ message: "Vector not found." });

        const queryResponse = await pineconeIndex.query({ vector: sourceVector, topK: 2 });
        const similarPinIds = queryResponse.matches.filter(m => m.id !== id).map(m => m.id);
        if (!similarPinIds.length) return res.status(200).json([]);

        const similarPins = await Pin.find({ _id: { $in: similarPinIds } });
        res.status(200).json(similarPins);
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: "Failed to fetch similar pins.", error: err.message });
    }
};
...
export const searchByImage = async (req, res) => {
    try {
        const imageFile = req.files?.media?.data;
        ...
        const clarifaiResponse = await new Promise((resolve, reject) => {
            stub.PostWorkflowResults(
                { workflow_id: "tag-and-embed", inputs: [{ data: { image: { base64: imageFile } } }] },
                metadata,
                (err, res) => { if(err) reject(err); else resolve(res); }
            );
        });

        if (clarifaiResponse.status.code !== 10000) throw new Error("Clarifai API error");

        const embedding = clarifaiResponse.results?.[0]?.outputs?.[1]?.data?.embeddings?.[0];
        if (!embedding) throw new Error("Vector generation failed");
        queryVector = embedding.vector;

        const queryResponse = await pineconeIndex.query({ vector: queryVector, topK: 2 });
        const similarPinIds = queryResponse.matches.map(m => m.id);
        if (!similarPinIds.length) return res.status(200).json([]);
        ...
        console.error(err);
        res.status(500).json({ message: "Failed to search by image.", error: err.message });
    }
};
