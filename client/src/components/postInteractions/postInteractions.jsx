...
// In src/components/postInteractions/PostInteractions.jsx
import Image from "../image/image";
import "./postInteractions.css";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import apiRequest from "../../utils/apiRequest";
...
const PostInteractions = ({ postId, pinOwner, imageUrl }) => {
    ...
    // 1. ADDED: QUERY FOR PIN DETAILS (to access title, description, link)
    const { data: pinData } = useQuery({
        queryKey: ["pinDetails", postId],
        queryFn: () => apiRequest.get(`/pins/${postId}`).then((res) => res.data),
        enabled: !!postId,
    });
    ...
    const deleteMutation = useMutation({
        mutationFn: deletePin,
        onSuccess: () => {
            // FIX: Invalidate the broader pins query key
    ...
    const handleDelete = () => {
        // UPDATED: Use pinData for a specific confirmation message
        const pinTitle = pinData?.title || 'this pin';
        if (window.confirm(`Are you sure you want to permanently delete "${pinTitle}"?`)) {
            deleteMutation.mutate(postId);
        }
        setMenuOpen(false);
    };
    ...
    const { isPending, error, data: interactionData } = useQuery({
        queryKey: ["interactionCheck", postId],
        queryFn: () => apiRequest.get(`/pins/interaction-check/${postId}`).then((res) => res.data),
    });
    ...
    const handleShare = async () => {
        try {
            // UPDATED: Use pinData for title/description in native share
            if (navigator.canShare && navigator.canShare({ files: [] })) {
                const file = await fetchImageAsFile(imageToShare);
                await navigator.share({
                    title: pinData?.title || "Check this pin!",
                    text: pinData?.description || "Found this amazing image on our website!",
                    files: [file],
                });
            } else {
                // Fallback for desktop: open WhatsApp/Facebook options
        ...
        <div className="postInteractions">
            <div className="interactionIcons">
                ...
                >
                    <path
                        d="M12 6.00019C10.2006 3.90317 7.19377 3.2551 4.93923 5.17534C2.68468 7.09558 2.36727 10.3061 4.13778 12.5772C5.60984 14.4654 10.0648 18.4479 11.5249 19.7369C11.6882 19.8811 11.7699 19.9532 11.8652 19.9815C11.9483 20.0062 12.0393 20.0062 12.1225 19.9815C12.2178 19.9532 12.2994 19.8811 12.4628 19.7369C13.9229 18.4479 18.3778 14.4654 19.8499 12.5772C21.6204 10.3061 21.3417 7.07538 19.0484 5.17534C16.7551 3.2753 13.7994 3.90317 12 6.00019Z"
                        stroke={interactionData.isLiked ? "#e50829" : "#000"}
                        strokeWidth="2"
                        fill={interactionData.isLiked ? "#e50829" : "none"}
                    />
                </svg>
                ...
                <div className="shareMenuContainer">
                    ...
                    {shareOpen && (
                        <div className="shareMenu">
                            {/* UPDATED: Use pinData for title in WhatsApp/Facebook share */}
                            <a
                                href={`https://api.whatsapp.com/send?text=${encodeURIComponent(pinData?.title || 'Check out this pin!') + ' ' + encodeURIComponent(imageToShare)}`}
                                target="_blank"
                                rel="noopener noreferrer"
                            >
                                Share on WhatsApp
                            ...
                            <a
                                href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(imageToShare)}&quote=${encodeURIComponent(pinData?.title || 'Check out this pin!')}`}
                                target="_blank"
                                rel="noopener noreferrer"
                            >
                                Share on Facebook
                ...
                <div className="moreMenuContainer">
                    ...
                    {menuOpen && (
                        <div className="moreMenu">
                            ...
                            {/* ADDED: Option to view original source link */}
                            {pinData?.link && (
                                <a 
                                    href={pinData.link} 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    className="sourceLinkButton"
                                >
                                    Visit Source Link
                                </a>
                            )}
